...
price_model <- lm(princ ~ cla + brd + iv.hp + iv.li + iv.wi + iv.he + iv.cy, data = data.observables)
data.observables$priceiv <- fitted.values(price_model)

model.2 <- lm(demand ~ priceiv, data = data.observables)
summary(model.2)
```
...
data$elasticity.own <- -alpha*(1-data$inside_market_share)*(data.observables$priceiv)
...
data$elasticity.cross <- alpha*(data$inside_market_share)*data.observables$priceiv
```
